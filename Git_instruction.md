...
![Логотип GitHab](GitHab.png)

**Git** и **GitHub** две разные вещи.
**Git** - это программа которую устанавливают на ПК и используют локально.

**GitHab** - это сервис компании Microsoft который позволяет интегрироваться с программой Git и настроить удаленную работу с вашими репозиториями.

## Работа с удаленныими репозиториями GitHab
...
git push
```
Если появиться ошибка! Ошибка может появиться в двух случая:
Если отпавляемой ветки нет в удаленной версии то использовать 1 команду. Либо нужно получить последнюю версию с удаленного репозитория 2 команда.
```Bash
1. git push -u origin branch_name
2. git pull
```
5. Провести изменения "c другого ПК"
6. Выкачать (pull) актуальное состояние из удаленного репозитория.
```Bash
git pull
```

## Копирование с уаленного репозитория
1. Делаем форк (fork) интересующего нас репозитория на свой аккаунт в Githab.
2. Мы делаем git clone для нашей версии этого репозитория. Копируем https ссылку.
```Bash
...
```Bash
Если при (push) на Github не появиться зеленый [Pull Request], надо перейти во вкладку вверху [Pull Request], потом нажать на New [pull request], в нашем предлагаемом репозитории выбираем нужную ветку, после появиться [Creat pull request] и Готово!
```