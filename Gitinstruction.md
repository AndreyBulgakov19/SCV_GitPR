...
Если кнопка не появилась,что бывает,то тогда нажимаем `Pull request` на строке выше где команды `Code,Actions` и т.д.Далее выбираем кнопку `New pull request` и в вкладке `compare`
выбираем новую ветку(в которой и делали изменения).Затем нажимаем зелёную кнопку `Create pull request`.
Также оставляем коментарий к работе.
