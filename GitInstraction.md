...
### 2.1 Создание удаленного репозитория (GitHub)
Чтобы создать свой репозиторий, необходимо нажать на зеленую кнопку `New`, в разделе `Repositories` сайта [GitHub](https://github.com/).
После открытия страницы создания, нужно заполнить поля:
* `Repository name` – имя репозитория. Стоит давать как можно более содержательные имена своим репозиториям.
* `Description` – описание. Необязательное поле. Его заменой может быть файл `README`.
* `Public/Private` - приватность репозитория, его открытость или закрытость (доступность только вам и людям, которым вы предоставите доступ).
* Последние три поля предлагают добавить, README-файл, .gitignore файл и выбрать лицензию для нашего проекта.

Завершим процесс создания репозитория, нажав кнопку `Create repository`.

### 2.2 Добавление/удаление удаленного репозитория к локальному
...
```
### 2.4 Получение изменений из удаленного репозитория
Чтобы получить изменения из удаленного репозитория, в Git предусмотрена команда:
```Bash
git fetch <имя репозитория>
```
Связкой `git fetch` и `git merge` можно получить изменения из удаленного репозитория и обновить свою рабочую копию. Комбинацией этих команд является выражение:
```Bash
git pull <имя репозитория>
```
### 2.5 Отправка изменений в удаленный репозиторий
Загружать изменения в url-репозиторий можно по команде:
```Bash
...
Но если вы вдруг передали флаг `--force`, то все коммиты, которых нет в вашем локальном репозитории, будут удалены. То есть удаленный репозиторий станет точной копией вашего локального. Есть более щадящая версия этого флага – `--force-with-lease`. Он будет делать в точности то же самое, но если вдруг нужно будет удалить коммит, созданный не вами, выполнение немедленно прекратится и вернется ошибка. Таким образом, вы не попадете в ситуацию, когда вы случайно удалили, к примеру, чужой коммит.

## 3. Работа с чужими удаленными репозиториями
### 3.1 Создание fork'ов репозиториев
Работа с чужими репозиториями (в т.ч. с теми, к которым нет прямого доступа), осуществляется через создание форков - копий репозиториев, с переносом их в ваш аккаунт GitHub.
Создать форк можно на странице интересуемого репозитория, нажатием на кнопку `Fork` вверху справа.

### 3.2 Работа с fork'ами
Работая с чужими репозиториями, следует создать свою ветку/ветки. Это позволит сохранить принцип параллельности. Автор репозитория сможет отдельно от продукта в целом, оценить целесообразность вклада "форкающего".

### 3.3 Публикация изменений в чужой удаленный репозиторий
...
или
```Bash
git push --set-upstream <имя репозитория> <имя ветки>
```
GitHub подскажет, что наша ветка опережает ветку исходного репозитория на один коммит и предлагает сделать пулл-реквест.
Нажимаем на кнопку `Compare` на подсказке GitHub, либо переходим на вкладку `Pull Requests` и нажимаем `New pull request`.

Открывшаяся страница позволит посмотреть внесённые изменения и выбрать две ветки: одну в исходном репозитории, на нее будут залиты наши изменения, вторую – в нашем репозитории, с нее будут скачаны изменения. Как только мы выбрали ветки и убедились, что не внесли никаких лишних изменений, нажимаем кнопку `Create pull request`.

На новой странице описываются изменения в проекте.
Как только мы закончили с описанием, можно нажимать кнопку `Create pull request`.

Новая страница покажет, как будет выглядеть пулл-реквест для владельца проекта. На этой же страницы будут комментарии/переписка по предложенным изменениям.
