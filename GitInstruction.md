...
-   **Слияние через терминал, при помощи удаленного репозитория GitHub:**
   
    Чтобы разрешить конфликт через терминал, необходимо вручную скорректировать версию файла, удалив все лишнее, далее добавить необходимые файлы в индекс и совершить коммит, способом [описанным выше](#5-фиксация-изменений).

    После этого необходимо слить ветвь в удаленный репозиторий, при помощи команды
    ```
    git push <remote-name> <branch-name>
    ```
    Если в удаленном репозитории не существует данной ветви, то её можно создать автоматически при помощи дополнительного параметра
    ```
    git push --set-upstream <remote-name> <branch-name>
    ```
    Перейти в репозиторий на гитхабе, создать pull-request, снизу страницы найти кнопку **Merge Pull Request**, нажать на стрелочку справа и выбрать опцию ***Create a merge commit***, которая автоматически сольет сделанные изменения в базовую ветвь. После чего вам будет предложено удалить ветвь в которой были сделаны изменения.

    Готово.

    Осталось только загрузить изменения в локальный репозиторий, и удалить ненужную локальную ветвь при помощи команд.
    ```
    ...
    Удалить удаленную ветвь можно при помощи команды
    ```
    git push -d <remote-name> <branch-name>
    ```

## 11. Удаление ветви
Перед тем как удалять ветвь, нужно перейти с неё на другую ветвь, потом удалить при помощи команды:
```
...
-   Написать сообщение
-   Отправить владельцу репозиторя на подтверждения при помощи кнопки **Create Pull Request** 