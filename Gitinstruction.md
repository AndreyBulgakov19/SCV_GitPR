...
```
  ...
  **13.5.3 Создаём новую ветку данного склонированного репозитория**
  ```
  git switch -c <name branch>
  ```
  **13.5.4 Добавляем, редактируем, фиксируем.**
  ```
  git commit -am "коммент"
  ```
  **13.5.5 Пушим к себе в удалённый репозиторий**
  ```
  git push
  ```
  **13.5.6 Отправляем хозяину репозитория на проверку**
