...
git init
```
## 1.5. Запись изменений в репозиторий (git status, git add, git commit, git diff)
...
## 1.6. Просмотр истории коммитов (git log)
...
## 1.7. Перемещение между сохранениями
...
## 1.10. Слияние веток и разрешение конфликтов
...
https://github.com/
## 2.2. Создать локальный репозиторий.
## 2.3. Создать удалённый репозиторий на GitHub:
![Скриншот_создать_репозиторий](createrepository.png)
## 2.4. Связать удалённый репозиторий с локальным.

Добавить удалённый репозиторий к проекту:
```
git remote add <repository_name> <repository_url>
```
Чтобы посмотреть список удаленных репозиториев, можно использовать команду:
```
git remote
```
...
git pull
```
При наличии конфликтов слияния, можно также использовать возможности `VSCode` для из разрешения и зафиксировать изменения.

## 2.5. Добавление изменений из локального репозитория в удалённый.

После внесения изменений в файл в локальном репозитории для их отправки в удалённый репозиторий используется команда:
```
git push
```
Также в случае, если изменения сохранены в локальном репозитории в ветке, которая не существует на удалённом репозитории, используется команда:
```
git push -u
```
или 
...
```
## 2.6. Работа с чужими репозиториями
В своем аккаунте не GitHub нужно сначала создать копию репозитория из другого аккаунта с помощью кнопки **`Fork`**.

Далее также клонируем копию репозитория на локальный компьютер и вносим необходимые изменения. После чего отправляем обновлённые данные в удалённый репозиторий на своём аккаунте (см. разделы 2.4-2.5).

**Важно:** Чтобы предложить изменения для чужого проекта, нужно использовать отдельную ветку - так автор сможет оценить изменения и слить их со своими данными, если он с ними согласен.

Чтобы предложить изменения автору проекта, на своём аккаунте в копии его репозитория (куда уже отправили изменения из локального репозитория) выполняем **`Pull Request`**: 

![Скриншот](pullrequest.png)

При сравнении версий обязательно выберите нужную ветку, где были сделаны изменения, затем при появлении данных для отправки нажмите `Create Pull Request`. Если необходимо, можно изменить название изменений (по умолчанию - комментарий к последнему коммиту) и заполнить описание.
