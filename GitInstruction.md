...
## 16. Создаём удалённый репозиторий на GitHub

На GitHub новый репозиторий создайтся он при помощи кнопки `**+**` - `New Repisitory`

Задаём название нового репозитория, его характеристики - Публичный или Приватный.

## 17. Связываем локальный и удалённый репозиторий.

Чтобы добавить удалённый репозиторий к проекту, необходимо выполнить группу команд:
```c#
git remote add origin "адрес репозитория в сети"
git branch -M main // Переименовываем текущую ветку в main (предполагается что мы находимся в ветке Master)
git push -u origin main // Отправляем ветку во внешний репозиторий
```

## 18. Fork

`Fork` (вилка) - создаёт копию удалённого репозитория (как правило стороннего) в вашем аккаунте.

После этого вы можете клонировать себе Форкнутый репозиторий и произвести в нём свои собственные изменения.

При создании форка необходимо выбрать, будтете ли вы копировать все ветки или только ветку `main`.

Изменения лучше всего производить в новой - "черновой" ветке.

## 19. PullRequest

`PullRequest` - это запрос одного разработчика забрать ветку (например вашу "черновую" ветку) из вашего репозитория в репозиторий другого разработчика.

На GitHub это вполне осуществляется через веб-интерфейс.

Сторонний разработчик может принять ваши изменения или отвергнуть, а так же написать к ним комментарий.

До тех пор пока ваш PullRequest не был одобрен - вы можете удалить его из запросов.