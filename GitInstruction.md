...
3. Команда `git push` - отправка изменений из локального репозитория на удалённый

```bash
git push -u origin main
```

При повтороной отправке (после свзяи) ключ `-u` не обязателен.

Для помещения текщуего коммита в ветку `main` репозитория `origin`

### 5.7.4 Просмотр удалённых репозиториев

Для просмотра удалённых репозиториев и связи их с локальными используется команда `git remote`. 

Для детальной информации используется ключ `-v`

```bash
git remote -v
```

### 5.7.5 Получение данных из удаленного репозитория

Для получения данных используется команда `git pull` - данная команда получает данные из удаленного репозитория и выполняет `merge`

### 5.7.6 Совместная работа с чужими удаленными рапозиториями

1. Создание своей копии для внесения изменений

Чтобы создать свою копию (черновик) в которую можно вносить изменения для дальнейшего слияния нужно сделать форк (fork) в нужном репозитории на  GitHub. С поможью кнопки "Fork" (справа вверху). Затем ввести имя своего форка. После этого необходимо созданный форк клонировать на локальный компьютер

2. В своем локальном репозитории (форке) создать новую ветку, в которую будеут вноситься изменения (`git branch`) переходим в нее (`git switch`). Не забываем про индексацию, особенно, если добавляли новые файлы.

3. Вносим изменения, добавляем файлы и т.д. Затем делаем коммит `git commit` (в комментарии крайне желательно описать сделанные изменения)
4. После этого помещаем сделанные изменения в свой удалённый репозиторий (форк) - `git push` 
5. В веб-интерфейсе своего форка  идем на закладку "**Pull requests**" и нажимаем зеленую кнопку "**New pull request**" выбираем ***НОВУЮ*** ветку (п.2) справа сверху.