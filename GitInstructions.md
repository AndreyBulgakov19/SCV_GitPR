...
* To make a copy of the existing repository type ***git clone <<u>URL</u>>***. This method is described in the **chapter 15** with more details.
...
### - git reset
  ...
  More detailed information is available in the documentaion on [git reset](https://www.atlassian.com/git/tutorials/undoing-changes/git-reset) documentaion

### - git revert
  The ***git revert*** command can be considered an 'undo' type command, however, it is not a traditional undo operation. Instead of removing the commit from the project history, it figures out how to invert the changes introduced by the commit and appends a new commit with the resulting inverse content. This prevents Git from losing history, which is important for the integrity of your revision history and for reliable collaboration.

    Note that git revert operation in its turn can be reverted by the same command.
More detailed information is available in the documentaion on [git revert](https://www.atlassian.com/git/tutorials/undoing-changes/git-revert)
...
### - git clean
  ***Git clean*** can be considered complementary to other commands like git reset and git checkout. Whereas these other commands operate on files previously added to the Git tracking index, the git clean command operates on untracked files.
  ...
  More detailed information is available in the documentaion on [git clean](https://www.atlassian.com/git/tutorials/undoing-changes/git-clean)
...
### git commit --amend
  ...
  More detailed information is available in the documentaion on [git commit --amend](https://www.atlassian.com/git/tutorials/rewriting-history#git-commit--amend)
...
### - git rebase
  ...
  More detailed information is available in the documentaion on [git rebase](https://www.atlassian.com/git/tutorials/rewriting-history/git-rebase)

## 16. Working with GItHub and execution of ***pull request***
> In order to follow the next steps the GitHub accaunt should be registered first (see [Sign up for GitHub](https://docs.github.com/en/get-started/signing-up-for-github/signing-up-for-a-new-github-account)).

### 16.1 Creating a repository on GitHub

1. Create a remote repository on GitHub:
  1.a. click on _"New repository"_ -> fill in the _"Repository name"_
  2.b. Select the access type of new repository (_private_ or _public_ - default);
  3.c. Initialize repository with README file (optional);
  4.d. Click on the "Create repository" button.
> The browser window will look like in the screenshot bellow:
![rep_img](RepositoryCreation.png)
2. Create local repository (***mkdir, git init*** commands):
  2.a. Create a README.md file (optional);
  2.b. Add some content into README.md file;
  2.c. Move the file into stage area of the repository and commit the change (***git add, git commit*** commands);
  2.d. If necessary rename the master branch of the local repository by ***git branch -M <<u>_new name_</u>>*** command;
  For example:
  ``` git branch -M main ```, where -M option implicitly implies forced mode (-f option);
3. Create remote repository on GitHub (_github.com_);
4. Link the remote repository with the local repository (_github.com_):
  4.a. Run the command ***git remote add origin <<u>_URL to the remote repository_</u>>*** ;
 >  **(make sure that URL is correct and there are no additional letter in the front and in the end of the URL)**
  4.b. To check the link type ***git remote -v***; There should be 1 link for push request and 1 link for fetch request
...
Following push request may be executed without any parameters (if the upload is going into the main repository);
...
### Three steps of forking the project:
#### - Fork the repository:
...
![Screenshot_of_banner.](https://docs.github.com/assets/cb-34106/images/help/pull_requests/pull-request-compare-pull-request.png)
       ...
       6.  To create a pull request that is ready for review, click  **Create Pull Request**. To create a draft pull request, use the drop-down and select  **Create Draft Pull Request**, then click  **Draft Pull Request**. For more information about draft pull requests, see "[Pull_requests](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/about-pull-requests#draft-pull-requests)."
