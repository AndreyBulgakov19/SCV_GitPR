...
Для работы с внешним репозиторием необходимо зарегистрироваться на этом ресурсе, например на `github.com`. 
### 12.1 Создание внешнего репозитрия
* Создаем внешний репозиторий в сервисе `github.com`, после, на странице созданного репозитрия будут указан несколько вариантов действий: 
    1. Выполняем пункты 4 и 5 данной инструкции для создания локального репозитория и создания точки сохранения (`git commit`).
    2. Выполняем команды
    ```
    git remote add origin https://github.com/<userName>/SCV_GitPR.git
    git branch -M main
    git push -u origin main
    ```
    3. Импортировать код из другого репозитория

### 12.2 Копирование (клонирование) внешнего репозитория на локальный ПК
...
### 12.3 Обновление локального репозитория данными с внешнего
...
git pull
```
Команда `git pull` - составная команда, она скачивает данные с внешнего репозитория и объединяет информацию. Во время объединения могут происходить конфликты, как с ними работать - описано в п.10 
### 12.4 Отправка измененных данных в внешний репозиторий
Для  отправки измененных данных необходимо сделать сохранение (п.5) и набрать команду 
```
git push
```  
Во время отправки или получения могут появиться ошибки, но git подсказывает что возможно сделать.
...
### 13.2 Отправка запроса на обновление чужого репозитория
После отправки измененных данных в Вашу копию чужого репозитория в сервисе `github.com`, там появится кнопка ***Compare & pull request***, при нажатии на которую создается запрос автору чужого репозитория для проверки и добавления изменных данных, с предварительным вводом сообщения об изменении.