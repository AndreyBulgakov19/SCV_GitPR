...
3. Перейти в загруженную папку командой:
```
cd <имя загруженной папки>
```
4. Создать новую ветку для работы с репозиторием

После внесения изменений в проект у себя, их можно предложить автору проекта. Это делается с помощью pull реквестов. Репозиторий должен быть выгружен на GitHub.
После выполнения команды `git push` у нашего репозитория на GitHub появится кнопка `Compare and pull request`. Просто следуем за зелеными кнопками.

Если кнопки нет, чтобы сделать Pull request, необходимо:
