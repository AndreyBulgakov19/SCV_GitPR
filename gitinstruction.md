...
### 12.5. Работа с чужими репозиториями

`GitHub` позволяет работать с чужими репозиториями. Но для того, чтобы вносить туда свои изменения, мы должны скопировать этот репозиторий к себе. Это делается с помощью команды `Fork`.

После внесения всех изменений у себя, мы можем отправить запрос на включение сделанных изменений (`Pull Requests`).
