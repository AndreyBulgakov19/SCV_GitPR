...
После того как мы зарегистрировались на сайте, нужно добавить удаленный репозиторий к существующему локальному. Для это необходимо восопльзовать следующей командой:
```
git remote add <название удаленного репозитория> <URL-ссылка на удаленный репозиторий>
```
Ссылку можно взять, нажав на большую зеленую кнопку Code на странице репозитория на GitHub.
![ScreenShot](2_git_fetch_pull_pus.png)

### 12.1 Клонирование удаленного репозитория.
Для выполнения этой операции в `Git` предусмотрена команда 
```
git clone <URL-ссылка на репозиторий>
```

### 12.2 Получение изменений из удаленного репозитория.
Если вы работаете в команде, вы будете по несколько раз в день обновлять свой репозиторий, загружая в него коммиты, сделанные другими учстниками проекта.
Чтобы получить изменения из удаленного репозитория, в `Git` предусмотрена команды:
```
git fetch
```

Существует команда позволяющая скачать все из текущего репозитория и автоматически сделать слияние `merge` с нашей версией. Это команда
```
git pull
```

### 12.3 Отправка изменений в удаленный репозиторий
Для загрузки на удаленный репозиторий свои локальные изменений используется команда
```
git push
```
***Важно! Все изменения нужно вносить в новой ветке.***

## 13. Форк репозитория на GitHub. Пулл-реквест.
```
Форк (fork) – точная копия репозитория, но в вашем аккаунте. Форки нужны, чтобы вносить свои изменения в проект, к репозиторию которого у вас нет прямого доступа.
```
```
Пулл-реквест (pull-request) – функция GitHub, позволяющая попросить владельца репозитория, от которого мы сделали форк, загрузить наши изменения обратно в свой репозиторий.
```
Форки и пулл-реквесты нужны, чтобы любой пользователь мог внести свой вклад в любой открытый проект, репозиторий которого есть на `GitHub`. Кроме того, перед тем как влить ваши изменения в основной репозиторий, ответственные обязательно проверят ваши корректировки и предложения.

### 13.1 Создание форка
Для начала зайдем на страницу интересующего репозитория. Нажимаем на кнопку `Fork`, как показано на картинке. После этого Git создаст точную копию этого репозитория в вашем аккаунте.
![fork](12_git_fetch_pull_pu.png)
 
 После успешного создания форка выполняем клонирование репозитория на локальный компьютер уже известной нам командой `git clone`. Вносим изменения, делаем коммит и выполняем команду `git push`, что бы отправить наши предложения в удаленный репозиторий.
 Теперь на GitHub будет отображаться, что наша ветка имеет коммиты и предлагает сделать пулл-реквест.

 ### 13.2 Pull-request.
 После того как GitHub предложил сделать пулл-реквест, нажимаем на кнопку `Compare`. Перед нами откроется окно создания пулл-реквеста.
 Здесь мы можем просмотреть внесенные изменения и выбрать две ветки: одну в исходном репозитории, на нее будут загружены наши изменения, вторую – в нашем репозитории, с нее будут скачаны изменения. Нажимаем кнопку `Create pull request`.
 Мы попадаем на страницу описания наших изменений. Здесь вносится описание изменений которые мы внесли, причины и т.д. Как только мы закончили с описанием, можно нажимать кнопку `Create pull request`.