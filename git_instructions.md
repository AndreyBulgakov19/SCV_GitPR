...
```
После этого можно будет проверить связь с удаленным репозиторием командой **git remote -v** .
...
На локальной машине используйте команду **git clone <скопированный_адрес>** . Сразу же будет скопирован и инициилизирован репозиторий на локальной машине.
...
В скопированном репозитории на GitHub (сделанный с помощью кнопки **Fork**) отобразятся изменения. Из него уже можно предложить изменения владельцу ориганального проекта (с которого снята копия) через кнопку **sd**. Кнопка может не отобразится и в этом случае нужно будет перейти в раздел **Pull requests** для предложения изменений.
